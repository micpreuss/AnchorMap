...
the primary run interface (`docker run anchormap:0.1.3 Rscript /opt/anchormap/anchor_map.R --config <yaml>`):
...
| Containers / envs | `rocker/r-ver:4.6.0` (pinned; = the validated host R — see the Phase-5 divergence note above) + `procps` + `USER root` + `ENTRYPOINT []`; image `anchormap:0.1.3` → Artifact Registry `us-central1-docker.pkg.dev/lencz-lab-cogent-1/docker-images/`; **referenced by version tag, never `latest`** |
