...
docker run --rm -v "$PWD:/work" -w /work \
    ghcr.io/micpreuss/anchormap:0.1.3 \
...
- Version tags such as `0.1.3` are the run interface; GHCR also carries `latest` as a convenient pointer
...
`ghcr.io/micpreuss/anchormap:0.1.3` and `ghcr.io/micpreuss/anchormap:latest`. GitHub Actions uses its
...
docker build --platform linux/amd64 \
    -t ghcr.io/micpreuss/anchormap:0.1.3 \
    -f docker/Dockerfile .

docker push ghcr.io/micpreuss/anchormap:0.1.3
```

Bump the tag (never reuse `0.1.3`) when the engine or a pinned dependency changes; update the tag in
